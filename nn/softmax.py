...
def softmax_loss(x, y):
    ...
    exps = np.exp(shifted)
    probs = exps / np.sum(exps, axis =1 , keepdims=True) #using broadcasting (N,C) / (N,1), again thanks to keepdims = True
    N = x.shape[0]
    loss = -np.sum(np.log(probs[np.arange(N), y]+1E-12)) / N  #indexing probs with y gives us the probability for the correct class in each example, which is the only one we care about for calculating loss.
